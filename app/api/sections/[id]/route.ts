...
type SectionBookOwner = { user_id?: string } | null;
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const owner = existing?.books as SectionBookOwner | undefined;
    if (error || !existing || owner?.user_id !== user.id) return NextResponse.json({ error: "Not found." }, { status: 404 });

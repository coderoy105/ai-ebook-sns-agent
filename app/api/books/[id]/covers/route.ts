...
async function loadBookAndCovers(bookId: string, userId: string, supabase: Awaited<ReturnType<typeof requireUser>>["supabase"]) {
...
async function responseCovers(bookId: string, userId: string, supabase: Awaited<ReturnType<typeof requireUser>>["supabase"]) {
  const loaded = await loadBookAndCovers(bookId, userId, supabase);
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await context.params;
    const { supabase, user } = await requireUser();
    const loaded = await loadBookAndCovers(id, user.id, supabase);
    if (!loaded) return jsonError("Book not found", 404);
    ...
    if (body.action === "regenerate") {
      ...
      const blueprint = blueprintRow?.blueprint && typeof blueprintRow.blueprint === "object"
        ...
        : {};
      const generation = Math.max(1, Math.floor(loaded.covers.length / 3) + 1);
      const concepts = createCoverConcepts({
        ...
        coreMessage: typeof blueprint.coreMessage === "string" ? blueprint.coreMessage : null
      }, generation);

      const { error: deleteError } = await supabase.from("book_covers").delete().eq("book_id", id);
      if (deleteError) throw deleteError;
      const rows = concepts.map((concept, index) => ({ book_id: id, concept, is_selected: index === 0 }));
      const { error: insertError } = await supabase.from("book_covers").insert(rows);

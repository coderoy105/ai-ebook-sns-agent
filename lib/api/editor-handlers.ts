...
export async function handleEditorGET(_:Request,path:string[]){
  try{
...
export async function handleEditorPATCH(request:Request,path:string[]){
  try{
...
export async function handleEditorPOST(request:Request,path:string[]){
  try{
    const {supabase,user}=await requireUser();
...
export async function handleEditorDELETE(_:Request,path:string[]){
  try{
    if(path[0]!=="outline"||path.length!==3)return bad("Unknown editor route",404);

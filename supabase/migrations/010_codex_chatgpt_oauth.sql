...
create table if not exists public.codex_connection_profiles (
  user_id uuid primary key references auth.users(id) on delete cascade,
  email text,
  plan_type text,
  selected_model text not null default 'gpt-5.6-luna',
...
alter table public.codex_connection_profiles enable row level security;
revoke all on public.codex_connection_profiles from anon, authenticated;
...
begin
  ...
  if existing_id is null then
    secret_id := vault.create_secret(
      ...
      'AI Book Studio Codex ChatGPT managed OAuth auth.json'
    );
